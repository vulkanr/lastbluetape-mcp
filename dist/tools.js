import { z } from "zod";
import { createList, listMyLists, getList, listItems, addItem, updateItem, setItemStatus, flagItem, getActivity, upgradeList, } from "./client.js";
export function registerTools(server) {
    server.tool("create_list", "Create a new punch list. Returns the list details, share token, and a shareable web URL.", {
    ...
    server.tool("list_my_lists", "List all punch lists linked to your API key. Also refreshes the token cache for list-level operations.", {}, async () => {
        const result = await listMyLists();
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("get_list", "Get details and stats for a specific punch list.", {
        list_id: z.string().describe("The list ID"),
    }, async (input) => {
        const result = await getList(input);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("list_items", "List all items in a punch list.", {
        list_id: z.string().describe("The list ID"),
    }, async (input) => {
        const result = await listItems(input);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("add_item", "Add a new item to a punch list. Returns the created item and list stats (including remaining capacity for free lists).", {
        list_id: z.string().describe("The list ID"),
    ...
    server.tool("update_item", "Update the title and/or description of a punch list item.", {
        list_id: z.string().describe("The list ID"),
    ...
    server.tool("set_item_status", "Change the status of a punch list item to open, in_progress, or done.", {
        list_id: z.string().describe("The list ID"),
        item_id: z.string().describe("The item ID"),
    ...
    server.tool("flag_item", "Flag or unflag a punch list item for attention.", {
        list_id: z.string().describe("The list ID"),
        item_id: z.string().describe("The item ID"),
    ...
    server.tool("get_activity", "Get the activity log for a punch list.", {
        list_id: z.string().describe("The list ID"),
    ...
    server.tool("upgrade_list", "Upgrade a punch list to Pro ($4.99 one-time). Requires LASTBLUETAPE_PAYMENT_METHOD env var with a Stripe payment method ID. Removes the 30-item limit and 90-day expiry.", {
